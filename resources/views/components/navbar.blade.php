@php
	$categoryNames = ['Nông trại', 'Sản xuất', 'Rang xay', 'Pha cà phê', 'Câu chuyện', 'Khác'];
@endphp
<div>
	<!-- main menu -->
	<div class="flex flex-wrap items-center justify-between space-x-3 sm:space-x-10 relative" x-data="{open: false}">
        ...
        <div>
            <a href="/" class="text-lg" title="home page">
                <x-logo class="w-16 h-16 sm:w-20 sm:h-20" />
            </a>
        </div>

        <div class="absolute right-0 top-0 h-screen w-1/2  bg-white flex flex-col px-6 pt-24 z-10 space-y-5 content-right transition sm:flex sm:relative sm:h-auto sm:w-auto sm:flex-row sm:h-auto sm:bg-transparent sm:pt-0 sm:px-0 sm:flex-grow sm:justify-around sm:items-center sm:content-center sm:space-y-0" :class="{'hidden': open === false}">
            @foreach ($categoryNames as $categoryName)
                <x-category-href :name="$categoryName"
                    class="text-sm sm:text-lg uppercase sm:tracking-widest text-cyan-500 hover:underline text-right sm:text-center" />
            @endforeach
        </div>

        <div class="mobile-menu sm:hidden absolute right-0 top-0 mt-5 mr-5 z-20" @click="open = !open">
	...
	<hr class="border bg-gray-500 sm:mt-3">
</div>
